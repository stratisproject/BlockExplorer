...
namespace Stratis.Bitcoin.Features.AzureIndexer.Tests
{
    ...
    public class ChainBuilder
    {
        ...
        ChainIndexer _Chain = new ChainIndexer(KnownNetworks.TestNet);
        ...
        private void Add(Transaction tx, bool indexBalances)
        {
            ...
            if (!tx.IsCoinBase)
            {
                this._Tester.Indexer.Index(new TransactionEntry.Entity(null, tx, null, KnownNetworks.TestNet));
                if (indexBalances)
                {
        ...
        public void SyncIndexer()
        {
            ...
            foreach (Block b in this._UnsyncBlocks)
            {
                ...
                {
                    this._Tester.Indexer.Index(new[] { new TransactionEntry.Entity(tx.GetHash(), tx, b.GetHash(), KnownNetworks.TestNet) });
                }
